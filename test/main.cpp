...
int main(int argc, char *argv[]) {
	...
	// 调用linearsifting重排算法
	start=clock();
	auto ltreod = dd_new->dynamicReorder(functionality_new, ltvarMap, dd::DynamicReorderingStrategy::Sifting);
	end=clock();
	...
	for(int i=0; i<30; ++i)
	{
		ltreod = dd_new->dynamicReorder(ltreod, ltvarMap, dd::DynamicReorderingStrategy::Sifting);
